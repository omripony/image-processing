...
using namespace std;
unsigned char img1[NUMBER_OF_ROWS][NUMBER_OF_COLUMNS];
void First_Assignemnt()
{
    ...
    for (int i = 0; i < numRectangles; i++) {
        AddGrayRectangle(img1, points[i][0], points[i][1], transparencies[i], grayLevels[i]);
        // Save the image after each rectangle is added
        char filename[20];
        if (i % 2)
        {
            sprintf(filename, "grayImg_step%d.bmp", i / 2 + 1);
            StoreGrayImageAsGrayBmpFile(img1, filename);
        }
    }

    // Save the final image
    StoreGrayImageAsGrayBmpFile(img1, "grayImg_final.bmp");
